            ...
            <tbody>
                @foreach ($data as $item)
                    <tr>
                        ...
                        <td>{{ $item->prioty == 0 ? 'No' : 'Yes' }} </td>
                        <td>{{ date('d/m/Y h:i:s', strtotime($item->created_at)) }} </td>
                        <td>{{ isset($item->updated_at) ? date('d/m/Y h:i:s', strtotime($item->updated_at)) : 'Not update' }}
                        </td>
                        <td>
                            <div class="table-btns">
                                <a href="{{ route('admin.category.edit', ['id' => $item->id]) }}" class="btn btn-edit">
                                    <button type="button" class="btn btn-block btn-dark">Edit</button>
                                </a>
                                <a href="{{ route('admin.category.delete', ['id' => $item->id]) }}" class="btn btn-delete">
